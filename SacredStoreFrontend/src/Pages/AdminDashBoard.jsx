...
const AdminDashboard = () => {
  ...
  const fetchRecentOrders = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/orders/recent');
      setRecentOrders(response.data);
      setDashboardStats(prevStats => ({
        ...prevStats,
        totalOrders: response.data.length,
        totalRevenue: response.data.reduce((acc, order) => acc + order.amount, 0).toFixed(2)
      }));
    } catch (error) {
  ...
  const renderProducts = () => (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={() => { setSelectedProduct(null); setShowAddProduct(true); }}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-600">Customer Management</h3>
