...
const RazorpayPaymentButton = () => {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const { data } = await axios.post('http://localhost:8080/api/razorpay/create-order', { // <--- UPDATED URL
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          try {
            const verificationResponse = await axios.post('http://localhost:8080/api/razorpay/verify-payment', { // <--- UPDATED URL
            ...
            alert('Payment verification failed due to an error. Please contact support.');
          }
        },
        prefill: {
          name: user ? user.name : '',
